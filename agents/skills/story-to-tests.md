...
当已有 `story.md`，需要生成验收和逻辑测试；若是 UI story，应在 `ui.md`、`ui/` 产物或 UI 跳过说明存在后补充 UI 可观察检查。
...
3. 若存在 `ui.md` 或 `ui/` 产物，再提取可观察交互、状态和视觉验证点。
4. 若 story 不需要 UI，记录 UI 跳过说明，避免后续阶段等待不存在的 UI 产物。
